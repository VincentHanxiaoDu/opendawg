...
**Read a file:**
```bash
mcp call read_file --params "{\"path\": \"${TMPDIR:-/tmp}/example.txt\"}" \
  npx -y @modelcontextprotocol/server-filesystem "${TMPDIR:-/tmp}"
```

**Write a file:**
```bash
mcp call write_file --params "{\"path\": \"${TMPDIR:-/tmp}/test.txt\", \"content\": \"Hello world\"}" \
  npx -y @modelcontextprotocol/server-filesystem "${TMPDIR:-/tmp}"
```

**List directory:**
```bash
mcp call list_directory --params "{\"path\": \"${TMPDIR:-/tmp}\"}" \
  npx -y @modelcontextprotocol/server-filesystem "${TMPDIR:-/tmp}"
```
...
```bash
mcp tools npx -y @modelcontextprotocol/server-filesystem "${TMPDIR:-/tmp}"
```
...
```bash
mcp alias add tmp-server npx -y @modelcontextprotocol/server-filesystem "${TMPDIR:-/tmp}"
mcp call list_directory --params "{\"path\": \"${TMPDIR:-/tmp}\"}" tmp-server
mcp call read_file --params "{\"path\": \"${TMPDIR:-/tmp}/file.txt\"}" tmp-server
