    ...
    def create(config: LLMConfig) -> LLMClient:
        ...
        match provider:
            ...
            case 'azure_openai':
                ...
                # Create the Azure OpenAI client without azure_deployment
                # so Responses API routes to /openai/responses (not /openai/deployments/X/responses)
                ...
                # Derive small_model from deployment name (e.g. gpt-4.1 → gpt-4.1-nano)
                # This prevents graphiti from falling back to DEFAULT_SMALL_MODEL (gpt-5-nano)
                # which doesn't exist as an Azure deployment.
                deployment = config.model or 'gpt-4.1'
                small_model = deployment + '-nano'
