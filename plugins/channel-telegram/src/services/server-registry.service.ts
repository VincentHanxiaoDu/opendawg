...
export class ServerRegistry {
    ...
    private memoryStore: Map<number, ServerRecord[]> = new Map();
    private isMemoryMode = false;
    private defaultServers: DefaultServer[];
    private dbPath: string;

    constructor(defaultServers: DefaultServer[], dbPath = "/data/telegramcoder.db") {
    ...
    private ensureUserDefaults(userId: number): void {
        const existing = this.listByUser(userId);
        if (existing.length > 0) return;

        // Seed from default servers
    ...
    listByUser(userId: number): ServerRecord[] {
    ...
    listByUserWithDefaults(userId: number): ServerRecord[] {
        this.ensureUserDefaults(userId);
        return this.listByUser(userId);
    }

    getActive(userId: number): ServerRecord | null {
        this.ensureUserDefaults(userId);
        const servers = this.listByUser(userId);
        return servers.find(s => s.isActive) ?? null;
    }

    getById(userId: number, serverId: string): ServerRecord | null {
    ...
    add(userId: number, url: string, name?: string, makeActive = false, username?: string, password?: string): ServerRecord {
    ...
    setActive(userId: number, serverId: string): ServerRecord | null {
        const record = this.getById(userId, serverId);
    ...
    findByUrl(userId: number, url: string): ServerRecord | null {
        const servers = this.listByUser(userId);
        return servers.find(s => s.url === url) ?? null;
    }

    /** Get matching servers by ID prefix (for ambiguity detection) */
    findByIdPrefix(userId: number, prefix: string): ServerRecord[] {
        const servers = this.listByUser(userId);
